...
def test_login_accepts_email_in_username_field():
    svc, persistence = _service()
    real_hash = svc._hash_password("correct-password")
    persistence.get_by_email.return_value = MagicMock(id="C00001", username="cust1", password_hash=real_hash)
    token = svc.login(UserLoginDTO(username="cust1@example.com", password="correct-password"))
    ...
    persistence.get_by_email.assert_called_once_with("cust1@example.com")
...
def test_login_by_email_returns_valid_jwt_with_customer_role():
    svc, persistence = _service()
    real_hash = svc._hash_password("correct-password")
    persistence.get_by_email.return_value = MagicMock(id="C00001", username="cust1", password_hash=real_hash)
    token = svc.login_by_email("cust1@example.com", "correct-password")
...
def test_login_by_email_rejects_missing_email_or_password():
    ...
    with pytest.raises(ValueError):
        svc.login_by_email("cust1@example.com", "")
...
def test_login_by_email_rejects_wrong_password():
    ...
    with pytest.raises(ValueError):
        svc.login_by_email("cust1@example.com", "wrong-password")
