...
class BrokerModel(Base):
    __tablename__ = "divine_broker_users"
...
class persistenceBroker:
    def __init__(self, session_factory=SessionLocal):
        ...
        queries.setdefault("update_password", (
            'UPDATE divine_broker_users SET password_hash = :password_hash, last_updated_date = :last_updated_date '
    ...
    def _generate_unique_id(self, db: Session) -> str:
        for _ in range(50):
            candidate = "B" + str(random.randint(0, 99999)).zfill(5)
            if not db.query(BrokerModel).filter_by(id=candidate).first():
                return candidate
    ...
    def create_user(self, username: str, password_hash: str, created_by: str = None, email: str = None, phone: str = None, first_name: str = None, last_name: str = None) -> BrokerModel:
        with self._session_factory() as db:
            try:
                ...
                query = self._queries.get("create_broker")
                params = {
                    "id": new_id,
                    "username": username,
